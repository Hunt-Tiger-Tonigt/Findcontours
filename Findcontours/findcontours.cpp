...
int main()
{
	...
	img = cv::imread(picture, cv::IMREAD_GRAYSCALE);     //读取相片并存为8位单通道图像
	cv::threshold(img, img_edge, 128, 255, cv::THRESH_BINARY);     //参看：https://blog.csdn.net/kksc1099054857/article/details/75041215
	...
	{
		int idx = sortIdx[n];
		cv::cvtColor(img, img_color, cv::COLOR_GRAY2BGR);     //将灰度图转换为RGB图
		cv::drawContours(
			img_color,      //要绘制轮廓的图像
